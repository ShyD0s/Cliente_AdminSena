...
<div class="row justify-content-center">
    <div class="col-lg-8">
        <div class="mb-4">
            ...
            <p class="text-muted small mb-0">Modifique la información académica del aprendiz y su equipo de cómputo asignado.</p>
        </div>
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-4">
                            ...
                            <div class="input-group">
                                ...
                                <input type="text" class="form-control border-start-0 ps-0 @error('name') is-invalid @enderror" id="name" name="name" value="{{ old('name', $apprentice['name']) }}" placeholder="Ej. Ana María Restrepo" required style="border-radius: 0 10px 10px 0; padding: 0.6rem 0.75rem;">
                                @error('name')
                        ...
                        <div class="col-md-6 mb-4">
                            ...
                            <div class="input-group">
                                ...
                                <input type="email" class="form-control border-start-0 ps-0 @error('email') is-invalid @enderror" id="email" name="email" value="{{ old('email', $apprentice['email']) }}" placeholder="Ej. amrestrepo@sena.edu.co" required style="border-radius: 0 10px 10px 0; padding: 0.6rem 0.75rem;">
                                @error('email')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-4">
                            ...
                            <div class="input-group">
                                ...
                                <input type="text" class="form-control border-start-0 ps-0 @error('cell_number') is-invalid @enderror" id="cell_number" name="cell_number" value="{{ old('cell_number', $apprentice['cell_number']) }}" placeholder="Ej. 3127654321" required style="border-radius: 0 10px 10px 0; padding: 0.6rem 0.75rem;">
                                @error('cell_number')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <div class="col-md-6 mb-4">
                            ...
                            <div class="input-group">
                                ...
                                <select class="form-select border-start-0 ps-0 @error('course_id') is-invalid @enderror" id="course_id" name="course_id" required style="border-radius: 0 10px 10px 0; padding: 0.6rem 0.75rem;">
                                    <option value="" disabled>Seleccione ficha...</option>
                                    @foreach($courses as $course)
                                        <option value="{{ $course['id'] }}" {{ old('course_id', $apprentice['course_id']) == $course['id'] ? 'selected' : '' }}>Ficha {{ $course['course_number'] }} ({{ $course['day'] }})</option>
                                    @endforeach
                                </select>
                                @error('course_id')
                                    <div class="invalid-feedback">{{ $message }}</div>
                        ...
                        <div class="col-md-12 mb-4">
                            ...
                            <div class="input-group">
                                ...
                                <select class="form-select border-start-0 ps-0 @error('computer_id') is-invalid @enderror" id="computer_id" name="computer_id" style="border-radius: 0 10px 10px 0; padding: 0.6rem 0.75rem;">
                                    <option value="" {{ is_null(old('computer_id', $apprentice['computer_id'])) ? 'selected' : '' }}>Sin equipo (Dejar libre)</option>
                                    @foreach($computers as $computer)
                                        {{-- Allow selecting the currently assigned computer, or any unassigned computer --}}
                                        @if(is_null($computer['apprentice']) || $computer['id'] == $apprentice['computer_id'])
                                            <option value="{{ $computer['id'] }}" {{ old('computer_id', $apprentice['computer_id']) == $computer['id'] ? 'selected' : '' }}>{{ $computer['brand'] }} [{{ $computer['number'] }}]</option>
                                        @endif
                                    @endforeach
                                </select>
                                @error('computer_id')
